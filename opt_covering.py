...
def vote(centers, data, r, fraction_th, iter_th):
    ...
    iter_finished = 0
    winning_centers = []
    while filtered_data.shape[1] / data.shape[1] > fraction_th and iter_finished < iter_th:
...
def draw(ax, ts_phis, color, size):
...
def prepare_plot(cov_params: CoveringParams):
    fig, ax = plt.subplots()
...
def main_demo():
    ...
    ax = prepare_plot(covering_params)
    draw(ax, data, "b", 1.0)
    covering_coords = covering_params.covering_coordinates()
    draw(ax, covering_coords, "r", 3.0)

    winning_centers = vote(covering_coords, data, covering_params.r_max, fraction_th=0.3, iter_th=3)
